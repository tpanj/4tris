...
static void DrawGame(Color C1, Color C2, Color C3, int masterOffsetX = 0, int masterOffsetY = 0);         // Draw game (one frame)
...
// Draw game (one frame)
void DrawGame(Color C1, Color C2, Color C3, int masterOffsetX, int masterOffsetY)
{
        if (!gameOver)
        {
            // Draw gameplay area
...
//            offset.x = screenWidth / 2 + 4 * SQUARE_SIZE;
            ...
            DrawText(TextFormat("LINES:   %04i", lines), offset.x, offset.y + 20, SQUARE_SIZE/2, GRAY);
...
void UpdateDrawFrame(void)
{
    ...
    DrawGame(SKYBLUE, BLUE, DARKBLUE, -580);
//     DrawGame(PURPLE, VIOLET, DARKPURPLE, 400);
